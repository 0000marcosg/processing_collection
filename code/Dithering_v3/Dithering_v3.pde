...
void setup() {
  frameRate(1);
  size(1024, 1024);
  kitten = loadImage("gatomiguel.jpg");
  kitten.filter(GRAY);
  //image(kitten, 0, 0);
...
void draw() {
  ...
  imageMode(CENTER);
  newImg.resize(newImg.width+100, newImg.height+100);


  newImg = dithering(kitten, 1);
  for (int y = 0; y < newImg.height-2; y++) {
    for (int x = 1; x < newImg.width-2; x++) {
      color nc = color(#E3F57E);
      color c = newImg.get(x, y);
      if (c == -1) {
        fill(nc);
        ellipse(x+width/2-newImg.width/2, y+height/2-newImg.height/2, 1, 1);
        //set(x*width/2, y*height/2, nc);
      } else {
        stroke(nc);
        //line(x+width/2-newImg.width/2 + random(-3,3),y+height/2-newImg.height/2,x+width/2-newImg.width/2 + random(-3,3),y+height/2-newImg.height/2 + random(-3,3));
  ...
  //textSize(18);
  //text("dithering_v3_14/1/20.pde",width/2-newImg.width/2, height/2+newImg.height/2+60);
...
PImage dithering(PImage img, int f) {
  img.loadPixels();
  for (int y = 0; y < img.height-2; y = y+2) {
    for (int x = 1; x < kitten.width-2; x=x+2) {
